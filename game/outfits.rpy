...
init -1 python in jn_outfits:
    ...
    class JNWearable():
        ...
        def asDict(self):
            ...
            """
            # depurar
            # print("DEBUG: asDict unlocked type:", type(self.unlocked), "value:", self.unlocked)
            # return {
    ...
    class JNOutfit():
        ...
        def asDict(self):
            ...
            #     print("ERROR: Outfit", self.reference_name, "has invalid unlocked type:", type(self.unlocked))
            #     self.unlocked = False
            # return {
            #     "unlocked": self.unlocked
            # }
            ...
            # Atributos que se esperan como strings
            ...
            if hasattr(self, 'condition') and self.condition is not None:
                if callable(self.condition): # Si es una función/método
                    try:
                        condition_val = self.condition.__name__ # Guardar el nombre de la función
                    except AttributeError:
                        condition_val = "callable_condition_no_name" # Placeholder si no tiene __name__
                else:
                    condition_val = str(self.condition) # Convertir a string
            data_dict["condition"] = condition_val
            
            # Atributo 'type' (posiblemente un Enum)
            type_name_val = None
            if hasattr(self, 'type') and self.type is not None:
                if hasattr(self.type, 'name'): # Común para Enums
                    type_name_val = str(self.type.name)
                else:
                    type_name_val = str(self.type) # Fallback a conversión de string
            data_dict["type"] = type_name_val
            
            # Atributo 'bgm'
            bgm_val = None
            if hasattr(self, 'bgm') and self.bgm is not None:
                bgm_val = str(self.bgm) # Asegurar que sea un string
            ...
            if hasattr(self, 'shown_count'):
                try:
                    ...
                    # Handle cases where shown_count cannot be converted to int
                    # e.g., log an error or use a default value
                    # This depends on how shown_count is supposed to behave
                    # For now, if it's not convertible, it might be better to
                    # let an error occur or assign a known safe value
                    # If self.shown_count could be None, handle that
                    if self.shown_count is None:
                        data_dict['shown_count'] = 0 # Or appropriate default
                    else:
                        # If it's not None and not convertible, this indicates a deeper issue
                        # For the purpose of preventing recursion, if it's an object
                        # we should be getting a primitive from it
                        # Fallback or error logging might be needed here
                        # For now, we'll assume it should be an int or convertible
                        data_dict['shown_count'] = 0 # Fallback to prevent error, review needed
            else:
        ...
        def _m1_outfits__save(self):
            ...
            # depurar
            data = self.asDict()
            # print("Guardando outfit:", self.reference_name, data)

...
init python in jn_custom_music:
    ...
    # Metadata constants (code of MAS)
    _MT_TITLE = "title"
    _MT_ARTIST = "artist"
    ...
    _VALID_EXTENSIONS_FOR_METADATA = ['.mp3', '.ogg', '.opus'] # Extensions mutagen will try to parse
    ...
    class JNCustomMusicSelectionOption:
        ...
        def __init__(
            ...
            display_prompt,
            option_type,
            file_name # This will now store the Ren'Py playable path, e.g., "<loop ...>custom_music/song.mp3"
        ):
    ...
    _NATSUKI_PICK_MUSIC_DONE_QUIPS = [
        ...
        _("All good!"),
        _("There we go!"),
        _("And...{w=0.3} we're good!"),
        _("Okie-dokie!{w=0.2} Ehehe.")
    ]
    ...
    def _get_audio_file_object(filepath_abs):
        ...
        try:
            ...
            renpy.log("JN Custom Music: Error loading metadata for {0}: {1}".format(filepath_abs, e))
            pass # Failed to load
        return None, None
    ...
    def _get_display_name_from_metadata(audio_obj, ext_lower, default_name_part):
        ...
        if ext_lower == ".mp3":
            ...
            if not title_tags: title_tags = audio_obj.tags.get(_MT_TITLE, []) # Try lowercase as fallback
    ...
    def _get_loop_string_from_metadata(audio_obj, ext_lower):
        ...
        # Validate and construct Ren'Py tag
    ...
    def _scan_custom_music_files_with_metadata():
        ...
        # CUSTOM_MUSIC_FOLDER is "game/custom_music/" (path from basedir)
        # renpy.list_files() returns paths relative to gamedir (e.g., "custom_music/song.mp3")
        gamedir_scan_prefix = CUSTOM_MUSIC_FOLDER
        if gamedir_scan_prefix.startswith("game/"):
            gamedir_scan_prefix = gamedir_scan_prefix[len("game/"):] # Should result in "custom_music/"
        else:
        ...
        # Get supported extensions from jn_utils
...
label music_menu:
    ...
    $ music_title = _("Unknown track")

    python:
        success = False
        final_music_options = []

        # --- Scan for custom music files using new metadata function ---
        scanned_custom_tracks = jn_custom_music._scan_custom_music_files_with_metadata()

        # --- Populate final_music_options in desired order ---
